...
- Transcript tool failures are the first-line shapes `is_tool_failure` (`cakelib.py`) matches: the `Error` prefix every tool error carries, plus the unprefixed `Hook blocked tool execution` prefix `PreToolUse` hook denials are stored with. Synthetic `not executed:` outputs written by history repair or a correction turn are deliberately not counted as failures. Telemetry `was_error` is authoritative for telemetry-covered sessions.
