...
- Tool failure taxonomy categories are keyed to current tool error message wording (`src/clients/tools/*.rs`); filesystem denials are reported as `sandbox-blocked` under the same tool and line-start scoping as `is_tool_failure`, and `read-only path` matches the full write-validation message (`is read-only (added via --add-dir)` / `is in a read-only directory (added via --add-dir)`) on the Edit and Write calls that produce it, not the word `read-only`. If those messages or markers change, update `classify_tool_error` and `is_tool_failure` in `cakelib.py`.
