...
- Transcript tool failures are detected by the `Error` output prefix, plus the unprefixed `Hook blocked tool execution` shape hook denials are stored with (`is_tool_failure` in `cakelib.py`); telemetry `was_error` is authoritative for telemetry-covered sessions.
