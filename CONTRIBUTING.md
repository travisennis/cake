...
8. Push the branch and open a pull request with `just pr`. Pass `labels="type:...,area:..."`, `body=<file>`, and `issue=<number>` to apply labels, use a body file, and comment the PR URL back on the managed issue; without them the recipe fills from commits.
