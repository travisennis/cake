...
Use a Conventional Commit subject for the pull request title too, for example `feat(session): show timing breakdown`. GitHub uses the title as the default squash-commit subject for a multi-commit pull request. The required `Conventional PR Title` check reruns when a title changes; review any manually edited squash message in the merge dialog as well.
