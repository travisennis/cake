...
### Change-class matrix
  ...
  | Coverage or change-risk sensitive          | `just check-coverage`                                                                                               | Coverage threshold, CRAP regression, and the same CC gate, all from one instrumented run. The script cleans the coverage artifacts that may affect results, then stops before evaluating the threshold if profile data survived the clean or the report spells one file under two source roots, rather than print a verdict nobody can trust |
  | Regenerating `ci/cargo-crap-baseline.json` | `just change-risk-baseline`                                                                                         | Cleans the same artifacts, guards the exported LCOV the same way, then rewrites the baseline from a fresh run                                                                                                                                                                                                                                |
  | `scripts/**` Python tooling                | `just check-scripts`                                                                                                | The same suites CI's `changes` job runs: dependency sweep, profiling helper, binary-size baseline, change classifier, `just pr`, eval harness, session metrics, and the coverage artifact guard. Stdlib only; no credentials, network, or provider calls. Also inside `just check`                                                           |
