...
- **Redundant derivation**: one fact computed independently at 2+ sites --- a payload parsed twice, a path resolved by two functions. First confirm both sites answer the same *domain* question; similar names often hide different domains, and divergence is then correct. Where they match, enumerate every consumer and compare answers: divergence is a correctness or security finding, not Category 5 duplication. Untyped is not a defect: where the consumer reads prose, ask whether one that cannot --- control flow, exit codes --- needs the distinction.
