...
**`tools`**: Tool definitions and execution. Each built-in tool (Bash, Read, Edit, Write) defines its JSON schema for the API and its execution logic; entries are registered in a `ToolRegistry` that pairs each definition with its executor closure and dispatches calls by name. User-defined toolbox tools (`tb__*`, discovered by `config::toolbox`) register through the same registry via the `toolbox` submodule, whose executor spawns the trusted, unsandboxed tool executable with `TOOLBOX_ACTION=execute` (ADR-017). Tools validate that paths are within the working directory or allowed temp directories before operating. The `secure_temp_dir` submodule owns the validated per-process directory shared by Bash output artifacts and macOS Seatbelt profile files. The Bash tool also performs pre-execution command safety checks that block known-destructive commands (destructive git operations and dangerous `rm -rf`) before they reach the shell. The `scheduling` module partitions a turn's tool calls
into execution groups so same-path Edit/Write calls run sequentially in issue order while everything else runs concurrently (ADR-013); it replaced the `duplicate_guard` module *(deleted 2026-07-09)*, which rejected the second same-file mutation instead. Exported helpers: `ToolContext`, `format_tool_list_section`, `read_extract_path` (used by agent loop for skill path-watching).
