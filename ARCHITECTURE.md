...
**`tools`**: Tool definitions and execution. Each built-in tool (Bash, Read, Edit, Write) defines its JSON schema for the API and its execution logic; entries are registered in a `ToolRegistry` that pairs each definition with its executor closure and dispatches calls by name. User-defined toolbox tools (`tb__*`, discovered by `config::toolbox`) register through the same registry via the `toolbox` submodule, whose executor spawns the trusted, unsandboxed tool executable with `TOOLBOX_ACTION=execute` (ADR-017). Tools validate that paths are within the working directory or allowed temp directories before operating. The `secure_temp_dir` submodule owns the validated per-process directory shared by Bash output artifacts and macOS Seatbelt profile files. The Bash tool also performs pre-execution command safety checks that block known-destructive commands (destructive git operations and dangerous `rm -rf`) before they reach the shell. The `scheduling` module partitions a turn's tool calls into execution groups so same-path Edit/Write calls run sequentially in issue order while everything else runs concurrently (ADR-013); it
replaced the `duplicate_guard` module *(deleted 2026-07-09)*, which rejected the second same-file mutation instead. Exported helpers: `ToolContext`, `format_tool_list_section`, `read_extract_path` (used by agent loop for skill path-watching).
...
- `session` (`Session`, `SessionWriter`): In-memory session state with JSONL serialization, plus the shared writer that appends locked JSONL records.
...
- `hooks` (`HooksLoader`, `HookSource`, `LoadedHooks`, `HookEvent`, `HookCommand`): JSON loading of user-defined hook commands from `~/.config/cake/hooks.json`, `.cake/hooks.json`, and `.cake/hooks.local.json`. The runtime that executes them lives in the top-level `src/hooks.rs` module (Layer 1).
- `toolbox` (`ToolboxTool`, `ToolboxEntry`, `ToolboxFormat`): User-defined toolbox tool discovery and describe-protocol parsing. Scans `CAKE_TOOLBOX`/`--toolbox` directories (default `~/.config/cake/tools`) for executables, runs each with `TOOLBOX_ACTION=describe`, and parses JSON or text schemas. The execute half lives in `clients::tools::toolbox` (Layer 3).
...
6. **Session files are append-only JSONL**: A new session is created as a fresh `{uuid}.jsonl` file with a `session_meta` header record. Continuations acquire an advisory lock and append conversation, task, and hook records live as the agent produces them. The most recent session for a directory is selected by the `session_meta.timestamp` value among files whose header matches the working directory, not by filesystem mtime.
...
- **Tool execution**: `ToolRegistry::execute` in `clients/tools/mod.rs`
- **Toolbox tools**: discovery/describe in `config/toolbox.rs`; execute in `clients/tools/toolbox.rs`
