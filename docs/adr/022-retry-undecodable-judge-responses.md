...
- The agent path resolves the same failure differently and deliberately: [ADR-028](./028-retry-accepted-body-transport-failures.md) retries a transport failure while an accepted 2xx body is read but keeps an undecodable body terminal, because a complete but undecodable body is an envelope incompatibility rather than a transient connection failure.
