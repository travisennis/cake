...
Choose bounded observations for direct `bash` and `sh` invocations with literal arguments, including quoted paths and an optional `--`. Unsupported shell syntax receives an explicit uncollected-evidence marker and still goes to the judge. This recognizer supplies evidence; it never authorizes execution. The raw command remains unchanged. Read one regular UTF-8 file, at most 32 KiB, through the same in-process grants used by Read. Denied, missing, oversized, binary, or special files fail closed before the provider call. Bypass skips collection.
