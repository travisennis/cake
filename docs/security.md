...
The judge's `data-egress` class is its policy for transmission off the machine. It blocks a command that sends local data, history, or a remote-ref change to a remote destination: file uploads (`curl -T`, `curl --upload-file`, `curl --data-binary @file`, `wget --post-file`, `scp` and `rsync` to a remote), cloud sync and copy (`aws s3 sync`, `aws s3 cp`, `docker push`), package publication (`npm publish`, `cargo publish`), mirror and ref-deleting pushes (`git push --mirror`, and `git push <remote> --delete <ref>` with no merged-state guard in the same command), and indirect egress whose payload a substitution or pipe assembles. The judge reads three facts from the command: whether a payload leaves at all, what the payload is (a literal, a file the command names, or the environment or whole directory a flag selects), and the destination as spelled, which it cannot verify. A read-only remote query (`aws s3 ls`, `curl -sS --fail https://api.example.test/health`, `git ls-remote`,
`git fetch`), a dry run (`npm publish --dry-run`), and a `git push` without `--mirror`, `--force`, or `--delete` send no local payload and change no remote ref, so they get no code. Whether a user authorized a specific transfer is trusted context the judge does not receive, so an authorized upload of a reviewed file can be a false block; authorization-aware egress semantics belong to that trusted-context work, not this class. Sending a secret is this class and reading one is `credential-disclosure`. The legacy corpus (`src/clients/tools/corpus/commands.jsonl`) carries the class's blocked and allowed cases.
