...
The Bash preflight approves the command once, before spawn. A command that outlives its `timeout` yield window keeps running in an in-memory session under the same OS sandbox until it exits, is killed, reaches the hard wall clock, or Cake exits. `BashSession` only reads, lists, or terminates an already approved process and does not run the judge again. Cake retains the sandbox guard and process-group kill guard for the session's lifetime; normal exit, Ctrl-C, and SIGTERM tear down live groups. When the shell exits, Cake stops remaining members of its process group, including background children that redirected the captured pipes. A resumed transcript's old session IDs are invalid. Both Bash and BashSession are unavailable under the `read-only` policy because a yielded command can continue changing state after the original tool call. Removing a simple trailing `&` before preflight keeps the judged and executed text identical and prevents that shell operator from immediately orphaning a
child; process-group cleanup covers descendants independently of shell spelling.

The `sandboxed_bash_session_yields_and_completes_under_platform_policy` test exercises yield, poll, kill, and redirected background-child cleanup through the platform sandbox. It passed under macOS Seatbelt during issue #639 implementation; Linux Landlock runs the same test in Linux CI.
...
The judge's `data-egress` class is its policy for sending payload content off the machine. It blocks a command that transmits local data to a remote destination: file uploads (`curl -T`, `curl --upload-file`, `curl --data-binary @file`, `wget --post-file`, `scp` and `rsync` to a remote), cloud sync and copy that sends content (`aws s3 sync`, `aws s3 cp`, `docker push`), package publication (`npm publish`, `cargo publish`), a mirror push (`git push --mirror`, which sends every local ref and object), and indirect egress whose payload a substitution or pipe assembles. The judge reads three facts from the command: whether a payload leaves at all, what the payload is (a literal, a file the command names, or the environment or whole directory a flag selects), and the destination as spelled, which it cannot verify. A remote ref deletion (`git push <remote> --delete <ref>`, or its `:<ref>` spelling) sends no local content, so it stays `unknown-destructive` rather than this class. A read-only
remote query (`aws s3 ls`, `curl -sS --fail https://api.example.test/health`, `git ls-remote`, `git fetch`), a dry run (`npm publish --dry-run`), and a `git push` without `--mirror`, `--force`, or `--delete` send no local payload and change no remote ref, so they get no code. Whether a user authorized a specific transfer is trusted context the judge does not receive, so an authorized upload of a reviewed file can be a false block; authorization-aware egress semantics belong to that trusted-context work, not this class. Sending a secret is this class and reading one is `credential-disclosure`. The legacy corpus (`src/clients/tools/corpus/commands.jsonl`) carries the class's blocked and allowed cases.
