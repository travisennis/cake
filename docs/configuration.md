...
- `retry_budget_secs`: extra seconds one recovery attempt may consume beyond `timeout_secs`. Defaults to 15; `0` disables recovery entirely. A timeout or retryable transport/HTTP failure triggers at most one recovery attempt within a total operation deadline of `timeout_secs + retry_budget_secs` (45 seconds with defaults), never two full timeout periods. The recovery re-sends the same request, honoring `Retry-After` up to a 5-second cap. Valid verdicts, refusals, and malformed verdicts are never retried; exhausted recovery still blocks the command (fail-closed).
