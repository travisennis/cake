...
`cake bash check -- <command>` uses the judge path and prompt without executing or collecting referenced host files; script verdicts can differ from Bash tool preflight. Verdicts, allowlist overrides, and bypass exit `0`; timeout, network, auth, and rate-limit errors exit `2`; other transport errors, malformed verdicts, and refusals exit `1`; unknown judge models exit `3`.
