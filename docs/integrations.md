...
Records cover initialization, conversation-model API attempts, command-safety judge attempts, retries, tool calls, and summaries. They include timing and usage metadata but intentionally omit prompts, assistant text, raw tool-output bodies, judge command/reason/cwd values, provider response bodies, and credentials. Successful `api_attempt` records may include an optional `termination` object with Cake's provider-neutral `classification` and the provider's raw `provider_status` or `provider_reason` when supplied. A `retry_scheduled` record whose reason is `semantic_incomplete` identifies the zero-delay continuation described above. Consumers must tolerate these and other additional enum values or optional fields; older sidecars and providers that omit them remain valid. Sidecars are never used for continue, resume, fork, or session discovery.

A `judge_attempt` record describes one provider call made by the Bash command-safety judge. It identifies the resolved model, API type, request controls, configured deadline, zero-tool/tool-choice state, prompt byte counts, attempt and retry ordinal, optional HTTP status and provider request ID, provider-neutral usage and termination when supplied, and a terminal class. Its timing separates request construction, the HTTP request through response headers, response-body parsing, verdict parsing, and total elapsed time. Timeout and transport records retain elapsed time; unavailable usage remains `null` instead of becoming a zero-filled block. These records are additive operational metadata and do not replace the verdict or fail-closed compensation record.
...
`cake bash check --diagnostic -- <command>` additionally prints the complete effective system and user prompts, provider-transformed request JSON, resolved non-secret request controls, zero-tool configuration, parsed assistant response, usage and termination metadata, attempt timing, and verdict. This is an explicit sensitive diagnostic: prompts contain command text, paths, repository state, and the model-supplied reason and may contain secrets embedded in those values. Cake does not include its resolved API key, authorization headers, or configured provider headers in the diagnostic. The command remains inspection-only and does not execute the supplied shell command.
