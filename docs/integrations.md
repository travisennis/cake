...
`cake bash check -- <command>` uses the preflight path and prompt without executing, and reports the `TypeSafe` observation the decision rests on in both output modes. Verdicts, allowlist overrides, and bypass exit `0`; timeout, network, auth, and rate-limit errors exit `2`; other transport errors, malformed verdicts, and refusals exit `1`; unknown judge models exit `3`.
