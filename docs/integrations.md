...
Bash calls may also provide an optional `cwd`. Relative paths resolve from the invocation working directory; absolute paths are accepted only when they resolve to an existing directory inside that workspace. The selected directory applies to that call only and is used consistently by the child process, judge, repository digest, and sandbox-denial diagnostics. Omitting `cwd` preserves the invocation directory.
