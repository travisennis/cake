...
What remains: the judge-driven regression runner over the #106 corpus (#174) and the controlled evaluation harness (#84) are the open measurement prerequisites that justified this design; #67 (recommended judge setup) is tracked separately; verdict caching stays deferred until latency data exists. After the `Closes #72` pull request merges, the issue body's link to this plan must be updated from the active path to `docs/exec-plans/completed/llm-judge-command-safety.md`.
