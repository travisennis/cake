...
- [x] (2026-08-08) Milestone 2: judge settings (`judge_model`, `judge_timeout_secs` under `[tools.bash.judge]`) merged through `SettingsLoader` into `LoadedSettings.judge` with default timeout 30 (zero clamped to 1); `src/clients/judge.rs` delivers `JudgeClient` (single bounded call reusing the configured backend, whole call inside the timeout, default model = agent's model, `with_model_override` for `judge_model`), `JudgeVerdict`/`JudgeDecision`, `JudgeError` taxonomy (Timeout/Transport/Malformed/Refusal), and the `reason` argument on the Bash tool schema, description, and `BashExecutionArgs`. Stub-judge wiremock tests prove timeout/transport/malformed/refusal error types (including the OpenAI-style `refusal`-field-with-`stop` shape), verdict parsing (block/warn/allow, no-code allow, block/warn without code rejected, confidence range), request round-trip of command/cwd/reason, and the model override. Independent review (Codex + subagent) found four in-scope issues, all fixed: timeout
  now bounds the body-read/parse phases, refusal detection handles the `refusal`-field shape, block/warn verdicts require a code, and a zero `timeout_secs` is clamped. Verified: `cargo test judge` (19), `cargo test bash` (170), `cargo test settings` (43), `cargo clippy --all-targets --all-features -- -D warnings`, `just ci`.
- [x] (2026-08-09) Milestone 3: embedded default rubric (`src/clients/judge_rubric.md`, snapshotted), stable verdict-code vocabulary (enum `VerdictCode` with the ten codes, `as_str`/`FromStr` via a shared spelling table; block/warn/allow verdicts are validated against the vocabulary and severity classes --- unknown codes, a `warn` carrying a destructive-class code, a `block` carrying the warn-class code, and an `allow` carrying a code all fail closed), the `[tools.bash.judge] rubric_file` setting appending optional user rubric guidance (`build_judge_system_prompt`), a spawn-free `repo_state_digest` (git work tree walk + `HEAD` read; branch or detached), and `cake bash check -- <command>` under `src/cli/bash.rs` (same judge path/prompt; prints verdict/code/message/confidence/latency; never executes; verdict exits 0; judge errors exit per the CLI contract via typed `JudgeError` classification --- timeout/network/auth/rate-limit exit 2, other transport/malformed/refusal exit 1, missing
  model and unreadable rubric exit 3; honors `--model`/`--profile` via `CommandRunOptions`). The judge request serializes the untrusted command/cwd/digest/reason as a JSON object (not a markdown fence), so command text cannot break out and inject prompt instructions. Live verification against the configured backend: `git status`→allow, `git push --force`→block+`git-force-push`, `rm -rf ./node_modules`→block+`destructive-rm`, `rg -rn foo`→warn+`rg-replace-footgun`, all exit 0; a model that returned verdict JSON without `message` exited 1 with the fail-closed malformed message. Rubric strengthened (message required on every verdict, no markdown fences, no invented warn classes, stash guidance recommends `git stash apply` not `pop`, `rm` carve-out requires targets stay under `/tmp` after resolving `..`) and `parse_verdict` strips a single markdown code fence after live runs showed the default model wrapping JSON in fences and omitting `message`. Verified: `cargo test --all-features`
  (1183), `cargo clippy --all-targets --all-features -- -D warnings`, `just cc-check`, `just ci`. Three independent review passes (Codex) found and fixed: severity/code contradictions, missing `--model`/`--profile` fidelity, exit-code misclassification, rubric contradictions, the `rm` carve-out bypass, and the markdown-fence prompt-injection vector.
- [x] (2026-08-09) Milestone 4: allowlist and telemetry-logged emergency bypass. `[tools.bash.judge]` gains `allowlist` (exact raw-command strings, unioned across global+project) and `enabled` (default true) in `JudgeSettings`/`BashJudgeSettings`; `src/clients/judge.rs` gains the single judge-path decision point `evaluate_command` (bypass check via `judge_is_enabled(settings, env)`, the bounded judge call, and the exact-match allowlist override returning `JudgeOutcome::Verdict { verdict, overridden }` or `JudgeOutcome::Bypassed`); the `CAKE_JUDGE=off` bypass constant lives in the config layer. `cake bash check` renders through that path and short-circuits the bypass before any judge setup (model/rubric), and settings load skips judge-model validation while bypassed, so a broken judge config cannot defeat the escape hatch: an allowlisted block prints the original verdict plus `Overridden: allowlist`, a disabled judge prints `Verdict: bypassed` without making a judge call (exit 0; the
  bypass and override data feed Milestone 6's telemetry records). Stub-judge tests prove the allowlist overrides a block and still judges (mock `expect(1)`), a benign allowlisted verdict is unaffected, a non-matching block is not overridden, both bypass forms make no judge call (mock `expect(0)`), bypass wins over broken config, and `judge_is_enabled` respects both forms; the env value is injected, so the suite is hermetic under an ambient `CAKE_JUDGE=off`. Settings tests cover defaults (enabled true, empty allowlist), global+project allowlist union, `enabled = false`, absent-`enabled` default, and unknown-judge-model-with-`enabled=false` loading cleanly. Verified: `cargo test judge` (67), `cargo test bash` (190), `cargo test settings` (47), `cargo clippy --all-targets --all-features -- -D warnings`, `just cc-check`, and both test filters under `CAKE_JUDGE=off`. Telemetry record variants (`JudgeDecision`, fail-closed, bypass) remain Milestone 6 per the plan's split.
- [x] (2026-08-09) Milestone 5: the LLM judge is the Bash command-safety preflight. `ToolContext` carries a per-run `JudgeContext` (judge settings + the agent's resolved model + the `[[models]]` registry) attached by `create_session`; `bash_judge_preflight` runs before every non-empty Bash spawn and stays active under `danger-full-access` and `CAKE_SANDBOX=off`. A `block` prevents spawn and returns the judge's message as the tool error; a `warn` executes normally and prepends `NOTICE:` without changing exit status or classification; the bypass and an allowlist-overridden block run unannotated; and any judge failure (missing context, unresolvable model, transport/malformed verdict) blocks with the fail-closed message. `[tools.bash.judge] model` resolves through the shared `resolve_judge_client_config` (also used by `cake bash check`), with the bypass short-circuiting before any judge setup. `src/clients/tools/bash_safety/` is deleted and the #106 Phase A corpus moves to
  `src/clients/tools/corpus/commands.jsonl`; the judge-driven regression runner over that corpus is #174. Stub-judge wiremock tests prove warn-precedes-output, allow runs unannotated, block prevents execution with the reason, a missing judge context fails closed, and an unreachable judge fails closed. Milestone 6's #66 telemetry wiring landed with this milestone (review of PR #195): the preflight records one compensation event per call --- verdict + code + latency (including allowlist-overridden blocks with the override flag), the fail-closed class, or a `judge_bypass` event --- carried through the tool error path via `ToolError` so denials are observable even when the tool call fails. Judge messages are sanitized (control characters stripped, 1000-char cap) before entering the agent loop. #123 (denials in `task_complete.permission_denials`) remains Milestone 6.
- [x] (2026-08-10) Milestone 6: judge blocks and fail-closed denials in `task_complete.permission_denials` (#123); metadata-only judge telemetry events and counters (#66, delivered with Milestone 5). The agent loop now records a denial for every real judge block and every fail-closed denial, through the same `permission_denials` path hooks use: `src/clients/agent/agent_loop.rs` maps each judge compensation event to a label (`judge block: <verdict-code>` or `judge fail-closed: <failure-class>`) and pushes `Bash(call-1): <label>`; warn, allow, bypass, and allowlist-overridden block events map to no denial because the command ran. Two end-to-end agent tests (`judge_block_records_permission_denial_with_verdict_code`, `judge_fail_closed_records_distinct_permission_denial`) drive a stub judge through the real agent loop and assert the denial lands in `task_complete.permission_denials` with its verdict code / failure class; the block test also asserts the telemetry sidecar `compensation`
  record carries `judge_verdict` + `block:git-force-push` + latency with no command text. A unit test (`judge_denial_label_maps_only_real_denials`) pins the mapping, including the non-denial cases. Verified: `cargo test session_telemetry` (8), `cargo test denials` (2), `just session-metrics-check` (7), `just ci`.
- [x] (2026-08-10) Milestone 7: two-layer documentation, superseded-ExecPlan removal, verification, issue records, and the `Closes #72` pull request. `docs/security.md` now states the two-layer model in its enforcement section (judge as best-effort gate, fail-closed, allowlist as the only override, telemetry-logged bypass, accepted risks) and its Related-decisions list points to ADR-018 instead of the withdrawn ADR-015; `ARCHITECTURE.md` replaces the stale "deterministic command checks" phrasing with the judge preflight and adds ADR-018 to Related decisions; `docs/integrations.md` documents the Bash `reason` argument, `cake bash check` output and exit-code mapping, and the compensation-record shapes (`block:<code>`/`warn:<code>`/`allow` details, fail-closed classes, `overridden`); ADR-018's telemetry paragraph is reconciled with the delivered record (no `tier`/`confidence` fields); the superseded ExecPlan `docs/exec-plans/active/declarative-command-policy.md` is removed with `git rm`;
  this plan moves to `docs/exec-plans/completed/`. #72's acceptance notes are filled with evidence (issue comment plus checked body boxes); the board Status could not be moved to In Progress from this machine because the local `gh` token cannot reach Projects v2, so Status stays Blocked until the pull request merges. Verified: `cargo fmt --check`, `just ci` (all gates pass: rust-version, host `cargo check --all-features`, fmt, clippy strict and no-default-features, full test suite, coverage/CRAP, imports, deps, module size, instruction size, domain glossary), `git diff --check`, panache format + lint on the changed Markdown files. Linux gap: `just clippy-linux` cannot run locally --- the `x86_64-linux-gnu-gcc` cross compiler required by aws-lc-sys is not installed; the Linux CI matrix covers it, and this pull request touches no Rust code. The instruction-size gate pinned `docs/integrations.md` at the flat 1500-word default with no headroom; the mandated Bash section landed at 1643
  words after displacing \~90 words of redundancy, and `scripts/lint-instruction-size.py` gains an explicit 1700 budget for that document with the reason recorded.
- [x] (2026-08-11) Post-merge reconciliation: #72 is closed and its project Status is Done; its issue link points to this completed plan. #123's scope is confirmed delivered by merged PR #199 and the stale issue is closed. #67 is unblocked and Ready. Remaining operational hardening and productization are tracked through GitHub parent issue #206 and its ordered sub-issues instead of extending this completed implementation plan.
...
- Observation: probing blocked-command classes through the implementing agent's own Bash tool is self-defeating, because the guard (before deletion) and the judge (after default-on) scan the literal command text. Live probes of blocked classes must go through `cargo test` fixtures or the emergency bypass, not `cargo run` with a destructive command literal. (Confirmed during Milestone 3: even `cake bash check -- 'rg -rn foo'` run from the agent's own Bash tool triggers the old guard's `NOTICE` warning on the literal `rg -rn` text inside the tool call, because `check_rg_replace_flag` scans the original segment before shell-data stripping.)
- Observation: the live default model (deepseek-flash-high) routinely wrapped verdict JSON in markdown code fences and omitted `message` on allow verdicts (2 of 3 early probes failed closed as `JudgeError::Malformed`). Fixes in Milestone 3: the rubric now demands `message` on every verdict and forbids fences, and `parse_verdict` strips a single markdown code fence before repair. The contract is unchanged --- a model that still omits `message` fails closed. Evidence: after the fixes, 6 consecutive live `cake bash check` probes (allow/block/warn classes) all succeeded with exit 0.
- Observation: the per-function CC gate is a ratchet over `ci/cargo-crap-baseline.json`; adding a third arm to `Commands::run` (baseline CC 3) and writing 10-arm `match` ladders for the verdict-code vocabulary pushed new/regressed functions over the gate. The sanctioned fix is lookup tables and dispatch helpers: `Commands::dispatch` for the subcommand match, and a shared `CODE_SPELLINGS` table backing both `as_str` and `FromStr`. Evidence: `just cc-check` passes after both refactors.
- Observation: the `CAKE_JUDGE` bypass is process-global state read by the shared judge path (`evaluate_command`), so any test that mutates the real env var while other judge-path tests run in parallel corrupts them (a full-suite run failed 7 stub-judge tests with `Verdict: bypassed` output while an env-scoped test held `off`). Final design: `judge_is_enabled(settings, bypass_env)` takes the env value as a parameter, `evaluate_command`/`run_bash_check` read `CAKE_JUDGE` at the call site, and no test sets the env var --- the env-form test passes `Some("off")` directly, eliminating the race by construction. Related: the bypass short-circuits `cake bash check` before model/rubric resolution, so a broken judge config cannot defeat the escape hatch (test: `bash_check_bypass_short_circuits_broken_judge_config`).
- Observation: ADR-018's telemetry paragraph promised the judge record carries `tier` and `confidence`, but the delivered `compensation` record (Milestone 5, PR #195) carries only `detail` (`block:<code>`/`warn:<code>`/`allow`, or the fail-closed class), `latency_ms`, and `overridden`. The Milestone 6 acceptance does not require the extra fields, and `tier` lost its meaning when the redesign collapsed the escalation tiers into a single judge gate. Evidence: `CompensationEventTelemetry` in `src/session_telemetry.rs` vs `docs/adr/018-llm-judge-command-gate.md`. Reconciled at Milestone 7: ADR-018 now describes the delivered compensation-record shapes and the `permission_denials` recording instead of the promised fields.
...
- Decision (amendment, supersedes the "same provider in v1" wording above): `judge_model` is a `[[models]]` name resolved to that entry's full configuration --- the `default_model`/`--model` vocabulary --- so the override may select a different provider or reasoning family (the documented hardening path). Unknown names fail at load time, mirroring `default_model`. The same-family *default* is unchanged. Date/Author: 2026-08-09, Travis Ennis.
...
- Decision: One pull request per milestone, merged to `master` before the next milestone starts. Each milestone is independently verifiable (per `docs/workflow/exec-plans.md`) and additive; Milestone 7's single `Closes #72` PR remains the final delivery. Milestone PRs reference #72 without the closing keyword so the issue stays open. Rationale: keeps each review narrow, keeps `master` current, and lets #84's evaluation findings abort the plan cheaply if needed. Date/Author: 2026-08-08, Travis Ennis.
...
What was achieved: the LLM judge is cake's only non-sandbox command gate, delivered across six implementation milestones plus this closing one. `bash_safety` is gone, ADR-015 is withdrawn and superseded by ADR-018, the superseded declarative-policy ExecPlan is removed, and the four durable authorities (`docs/security.md`, `docs/configuration.md`, `docs/integrations.md`, `ARCHITECTURE.md`) describe the two-layer model. A user can inspect any command with `cake bash check -- <command>`, override a block with an exact-match allowlist, and escape a judge outage through the telemetry-logged bypass; judge blocks and fail-closed denials land in `task_complete.permission_denials`, and metadata-only compensation events land in the telemetry sidecar.

What remains outside this completed plan: #84 and #106, the original evaluation-harness and corpus prerequisites, are complete. The judge-specific regression runner (#174), request diagnostics (#202), stateless-context hardening (#203), bounded retry (#204), reliability SLO evaluation (#205), and setup productization (#67) are ordered as GitHub sub-issues under parent #206. Verdict caching remains deferred until measurement shows that its latency benefit justifies context invalidation and safety complexity. These are follow-on tasks with their own acceptance and branches, not unfinished milestones of #72.

Lessons learned: a stochastic gate is only acceptable with fail-closed defaults, metadata-only telemetry, an explicit escape hatch, and a measured regression corpus --- the plan's decisions held across all seven milestones. The ADR-018 telemetry prose drifted from the delivered record (`tier`/`confidence` promised, `detail`/`latency_ms`/`overridden` shipped); reconciling the decision record at the closing milestone caught it. The instruction-size gate pinned `docs/integrations.md` at the flat 1500-word default with zero headroom, so the mandated Bash-section documentation required a budget entry plus displaced prose; future contract additions should budget headroom up front.
...
The architectural source is `docs/adr/018-llm-judge-command-gate.md` (accepted in Milestone 1, 2026-08-08). The tracking issue is #72. The evaluation corpus is #106's `src/clients/tools/corpus/commands.jsonl` (moved from `src/clients/tools/bash_safety/corpus/commands.jsonl` when `bash_safety` was deleted in Milestone 5). The judge-driven runner over the corpus is #174 (open follow-up). Add concise test transcripts and final examples during implementation.
...
Revision note (2026-08-08, fifth): Milestone 2 landed on branch `feat/judge-client-settings`. Judge settings live at `[tools.bash.judge]` (model override + timeout, defaults 30s); the `JudgeClient` reuses the configured backend (`Backend::send_request`) with a bounded `tokio::time::timeout`, extracts the assistant text, and parses the verdict JSON with `repair_json_args` fallback. The Bash tool's `reason` argument is additive (schema, description, args struct) and consumed by the judge in Milestone 5. #72 stays board-Blocked pending #66 (Ready, unclaimed); per the ordering implications #66 gates only Milestone 5's default-on point, so Milestone 2 proceeded unblocked.

Revision note (2026-08-09, sixth): Review follow-up on the Milestone 2 PR corrected the `judge_model` semantics before they ship: `[tools.bash.judge] model` is a `[[models]]` name (the `default_model`/`--model` vocabulary) resolved to that entry's full configuration --- provider, base URL, API key, temperature, reasoning --- not a raw provider model identifier. Settings load now fails on an unknown judge model name (mirroring `default_model`), `with_model_override` takes the named model's full `ModelConfig`, and `docs/configuration.md` documents the keys.

Revision note (2026-08-09, seventh): Milestone 3 landed on branch `feat/judge-rubric-bash-check`. The embedded default rubric (snapshotted), the ten-code verdict vocabulary (unknown codes on block/warn fail closed), the `[tools.bash.judge] rubric_file` setting, the spawn-free `repo_state_digest`, and `cake bash check -- <command>` are all in. Live verification against the configured backend added two robustness fixes: the rubric now requires `message` on every verdict and forbids markdown fences, and `parse_verdict` strips a single code fence. `docs/configuration.md` documents the new `rubric_file` key. The issue body link to this plan is now a markdown link.

Revision note (2026-08-09, eighth): Milestone 4 landed on branch `feat/judge-allowlist-bypass`. `JudgeSettings` gains `allowlist` (exact raw-command strings; unioned global+project) and `enabled` (default true); `src/clients/judge.rs` adds the shared judge-path decision point `evaluate_command` returning `JudgeOutcome` (Verdict-with-override-flag or Bypassed) plus `judge_is_enabled(settings, env)`; the `CAKE_JUDGE=off` bypass constant lives in the config layer so settings validation can honor it; `cake bash check` renders overrides and bypass through that path, short-circuits the bypass before judge setup, and the loader skips judge-model validation while bypassed. `docs/configuration.md` documents `enabled`, `allowlist`, and the validation exception. Telemetry record variants are deferred to Milestone 6 as planned.

Revision note (2026-08-09, ninth): Milestone 5 landed on branch `feat/judge-bash-preflight`. The LLM judge is now the only non-sandbox command gate: every non-empty Bash command is judged before spawn through `bash_judge_preflight`, carrying the per-run `JudgeContext` on `ToolContext` (attached in `create_session`), with the bypass short-circuiting before any judge setup and judge failures blocking with the fail-closed message. `[tools.bash.judge] model` resolution is shared between `cake bash check` and the preflight via `resolve_judge_client_config`; `with_model_override` is removed now that the preflight consumes the shared resolution. `src/clients/tools/bash_safety/` is deleted (its `rg -rn`-warning tests are replaced by stub-judge gate tests) and the #106 Phase A corpus moves to `src/clients/tools/corpus/commands.jsonl`; #174's judge-driven runner over the corpus remains the open regression-runner follow-up. Telemetry record variants remain Milestone 6.

Revision note (2026-08-10, eleventh): Milestone 7 landed on branch `docs/judge-docs-closure`. Two-layer documentation is complete (`docs/security.md`, `ARCHITECTURE.md`, `docs/integrations.md`; `docs/configuration.md` was already current from Milestones 3/4), ADR-018's telemetry paragraph is reconciled with the delivered compensation record, the superseded ExecPlan is removed with `git rm`, this plan moves to `docs/exec-plans/completed/`, #72's acceptance notes are filled, and the closing pull request references `Closes #72`. The instruction-size gate required a budget entry for `docs/integrations.md` (1700) with \~90 words displaced; the `Artifacts` section now records the corpus's post-Milestone-5 location.

Revision note (2026-08-11, twelfth): Post-merge issue reconciliation is complete. #72 and delivered follow-up #123 are closed and marked Done, #67 is unblocked, and successor parent #206 owns the remaining ordered GitHub sub-issues. The Outcomes section now distinguishes the completed #72 implementation from later judge validation, hardening, and productization instead of treating this ExecPlan as a cross-issue program tracker.
