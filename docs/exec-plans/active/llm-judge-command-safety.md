...
- [x] (2026-08-08) Milestone 2: judge settings (`judge_model`, `judge_timeout_secs` under `[tools.bash.judge]`) merged through `SettingsLoader` into `LoadedSettings.judge` with default timeout 30; `src/clients/judge.rs` delivers `JudgeClient` (single bounded call reusing the configured backend, default model = agent's model, `with_model_override` for `judge_model`), `JudgeVerdict`/`JudgeDecision`, `JudgeError` taxonomy (Timeout/Transport/Malformed/Refusal), and the `reason` argument on the Bash tool schema, description, and `BashExecutionArgs`. Stub-judge wiremock tests prove timeout/transport/malformed/refusal error types, verdict parsing (block/warn/allow, no-code allow, confidence range), request round-trip of command/cwd/reason, and the model override. Verified: `cargo test judge` (16), `cargo test bash` (170), `cargo test settings` (42), `cargo clippy --all-targets --all-features -- -D warnings`, `just ci`.
