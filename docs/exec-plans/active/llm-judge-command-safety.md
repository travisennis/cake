...
- Observation: ADR-018's telemetry paragraph promises the judge record carries `tier` and `confidence`, but the delivered `compensation` record (Milestone 5, PR #195) carries only `detail` (`block:<code>`/`warn:<code>`/`allow`, or the fail-closed class), `latency_ms`, and `overridden`. The Milestone 6 acceptance does not require the extra fields, and `tier` lost its meaning when the redesign collapsed the escalation tiers into a single judge gate. Evidence: `CompensationEventTelemetry` in `src/session_telemetry.rs` vs `docs/adr/018-llm-judge-command-gate.md`. Reconcile the ADR-018 prose at Milestone 7.
...
Revision note (2026-08-10, tenth): Milestone 6 landed on branch `feat/judge-denials-telemetry`. The agent loop records judge blocks and fail-closed denials into `task_complete.permission_denials` through the same path hooks use, with a distinct label carrying the verdict code or failure class; warn, allow, bypass, and allowlist-overridden blocks are not denials. #66's counters and the metadata-only compensation events were already delivered with Milestone 5; this milestone added the #123 denial recording, two end-to-end agent tests (block and fail-closed, both asserting the emitted `task_complete` record and the telemetry sidecar), a unit test pinning the label mapping, and a Surprises entry recording the ADR-018 `tier`/`confidence` prose drift for Milestone 7.
