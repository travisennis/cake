...
When authoring an executable specification (ExecPlan), follow this document's guidance to the letter. If it is not in your context, refresh your memory by rereading this file. Be thorough in reading (and re-reading) source material to produce an accurate specification. When creating a spec, start from the skeleton and flesh it out as you do your research.
...
When an ExecPlan completes an issue, mirror the issue lifecycle in [docs/workflow/tasks.md](../workflow/tasks.md): fill the issue's acceptance notes when the change is pushed (the issue stays open); when the PR merges to `master`, update the ExecPlan Outcomes & Retrospective, move the ExecPlan to the completed bucket with `git mv`, update the issue's link to the completed plan path, then close the issue.
...
- When you discover optimizer behavior, performance tradeoffs, unexpected bugs, or similar findings that shaped your approach, capture those observations in the `Surprises & Discoveries` section with short evidence snippets (test output is ideal).
