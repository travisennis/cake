...
Read this before refactors, behavior-preserving cleanup, error-handling changes,
module moves, lint posture changes, or maintainability work that is not owned by
a narrower risk guardrail.
...
- Lint guarantees, including no production `unwrap`/`expect` and absolute
  `crate::` imports.
...
- Update `ARCHITECTURE.md` and implementation-location references when moving
  responsibilities between modules.
