...
- **`run(command, work_dir)`** --- validates the command through the same shell validator as the foreground tool, allocates an id from an `AtomicU64` seeded with `Utc::now().timestamp_millis()`, persists a `Running` record, then `tokio::spawn`s:
  ...
  - final record persisted Returns immediately: `"Background task {id} started: {command}"`.
...
Low priority. It also overlaps with GitHub Issues for this repo specifically.
...
## Not worth porting
  ...
  | Persistent task tools                       | `task_create` / `task_get` / `task_list` / `task_update` with dependency tracking. For this repo, GitHub Issues already occupies the slot                                        |
