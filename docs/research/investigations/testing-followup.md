...
**Reason for deferral:** Wiremock is designed for mocking HTTP responses, not simulating low-level network failures. Testing connection refused, DNS failures, and SSL/TLS errors would require either: 1. A trait-based HTTP client abstraction (significant refactoring) 2. Integration tests with real network conditions (slow, flaky)
