...
Without `CAKE_JUDGE_BENCH_MODELS`, this selects the effective configured judge model (judge override, then default model). Benchmark profile, repetitions, case-line selection, results-directory and concurrency overrides still apply. Concurrency for this leg is `CAKE_JUDGE_BENCH_CONCURRENCY` (default `1`); `CAKE_JUDGE_CORPUS_CONCURRENCY` belongs to `just judge-corpus` and has no effect here. Above `1` the wall clock drops and the latency SLO checks are not measured, so quote a latency only from a run at `1` (see `scripts/judge-bench/README.md`). Historical baseline revision: `f8fd404`. No live baseline was collected for #314; provider/spend authorization is separate.
