...
- `reason` is optional and is passed to the judge as the model's untrusted
  self-report. `tags` may contain `same-command-pair`, `reason-laundering`, or
  `reason-injection` for the corresponding judge-specific regression groups.
...
The threshold choice and initial baseline evidence are recorded in issue #174.
